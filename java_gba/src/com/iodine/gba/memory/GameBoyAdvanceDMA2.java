...
import com.iodine.gba.audio.GameBoyAdvanceSound;
...
public class GameBoyAdvanceDMA2 {
    ...
    private GameBoyAdvanceSound sound;
    ...
    public GameBoyAdvanceDMA2(GameBoyAdvanceIO ioCore) {
        this.IOCore = ioCore;
    }
    ...
    public void handleDMACopy() {
        ...
        if (this.enabled == 0x10) {
            this.copySound(source);
        } else {
    ...
    private void copySound(int source) {
        int data = this.memory.memoryReadDMAFull32(source);
        IOCore.wait.singleClock();
        IOCore.updateTimerClocking();
        this.sound.writeFIFOB32(data);
        this.soundDMAUpdate(source);
        this.DMACore.updateFetch(data);
    }
    ...
    private void soundDMAUpdate(int source) {
        this.wordCountShadow = (this.wordCountShadow - 1) & 0x3FFF;
        if (this.wordCountShadow == 0) {
            this.pending = 0;
            if (!this.repeat) {
                this.enabled = 0;
            } else {
    ...
    public int nextEventTime() {
        ...
        switch (this.enabled) {
            ...
            case 0x4:
                ...
                break;
            case 0x10:
                clocks = this.sound.nextFIFOBEventTime();
                break;
        }
        return clocks;
    }
